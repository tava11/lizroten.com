...
#> 1 FALSE 1109817 Pink Le~ pink-lemon~       72078 Amy Rapp        <df[,59] [2 x ~
#> 2 FALSE 1111323 Gift Of~ gift-of-ho~      111099 Bex Knitty Des~ <df[,59] [1 x ~
#> 3 FALSE 1110943 Intwine~ intwined-h~      101556 Tif Neilan - t~ <df[,59] [1 x ~
#> 4 FALSE 1111324 Gift Of~ gift-of-ho~      111099 Bex Knitty Des~ <df[,59] [1 x ~
#> 5 TRUE   891114 Rye Lig~ rye-light         45502 tincanknits     <df[,59] [3 x ~
#> 6 TRUE   130787 Hermion~ hermiones-~       14789 Erica Lueder    <df[,59] [2 x ~
```
...
#>   free       id name      permalink   designer.id designer.name  pattern_sources
#>   <lgl>   <int> <chr>     <chr>             <int> <chr>          <list>         
#> 1 FALSE 1111324 Gift Of ~ gift-of-ho~      111099 Bex Knitty De~ <df[,59] [1 x ~
```
...
#>  $ comments_count         : int 27
#>  $ created_at             : chr "2021/02/05 10:16:57 -0500"
#>  $ currency               : chr "EUR"
#>  $ difficulty_average     : int 0
#>  $ difficulty_count       : chr ""
#>  $ downloadable           : logi TRUE
#>  $ favorites_count        : int 581
#>  $ free                   : logi FALSE
#>  $ gauge                  : num 24
#>  $ gauge_divisor          : int 4
#>  $ gauge_pattern          : chr "In slip stitch pattern as instructed"
#>  $ generally_available    : chr "2021/02/01 00:00:00 -0500"
#>  $ id                     : int 1111324
#>  $ name                   : chr "Gift Of Honey Hat"
#>  $ pdf_url                : chr ""
#>  $ permalink              : chr "gift-of-honey-hat"
#>  $ price                  : num 1.69
#>  $ projects_count         : int 8
#>  $ published              : chr "2021/02/01"
#>  $ queued_projects_count  : int 57
#>  $ rating_average         : int 0
#>  $ rating_count           : chr ""
#>  $ row_gauge              : num 44
#>  $ updated_at             : chr "2021/02/06 10:00:46 -0500"
#>  $ url                    : chr "http://www.bexknittyworld.blogspot.com"
#>  $ yardage                : int 161
#>  $ yardage_max            : int 161
#>  $ personal_attributes    : chr ""
#>  $ sizes_available        : chr "Circumference: 8.5” / 22cm lying flat (17”/ 43cm all around),  Length: 8” / 20cm Length before decreases:  6.5”"| __truncated__
#>  $ product_id             : int 674550
#>  $ currency_symbol        : chr "\200"
#>  $ ravelry_download       : logi TRUE
#>  $ download_location      :List of 1
#>  $ pdf_in_library         : logi FALSE
#>  $ volumes_in_library     : chr ""
#>  $ gauge_description      : chr "24 stitches and 44 rows = 4 inches in slip stitch pattern as instructed"
#>  $ yarn_weight_description: chr "DK (11 wpi)"
#>  $ yardage_description    : chr "161 yards"
#>  $ pattern_needle_sizes   :List of 1
#>  $ notes_html             : chr "\n<blockquote>\n<p><strong>To celebrate its release, this pattern will be FREE until end of day (CET) 6th Febru"| __truncated__
#>  $ notes                  : chr "> **To celebrate its release, this pattern will be FREE until end of day (CET) 6th February 2021. If you intend"| __truncated__
#>  $ packs                  :List of 1
#>  $ printings              :List of 1
...
#>   ..$ id       : int [1:7] 3 9 10 186 212 286 306
#>   ..$ permalink: chr [1:7] "unisex" "teen" "adult" "mosaic" ...
```
...
#>   free       id name      permalink   designer.id designer.name  pattern_sources
#>   <lgl>   <int> <chr>     <chr>             <int> <chr>          <list>         
#> 1 FALSE 1109817 Pink Lem~ pink-lemon~       72078 Amy Rapp       <df[,59] [2 x ~
#> 2 TRUE   891114 Rye Light rye-light         45502 tincanknits    <df[,59] [3 x ~
#> 3 TRUE   130787 Hermione~ hermiones-~       14789 Erica Lueder   <df[,59] [2 x ~
#> 4 TRUE   580119 Rose Cit~ rose-city-~       82613 Mara Catherin~ <df[,59] [1 x ~
#> 5 TRUE   315418 Vanilla ~ vanilla-la~       53376 Virginia Rose~ <df[,59] [1 x ~
...
```
