...
options()
## data ----
nokomis <- rgdal::readOGR("data/workshop_data/104606_Regional_Parks_Summer_2018_HW_workshop_zone_activity/104606_Regional_Parks_Summer_2018_HW_workshop_zone_activity.shp") %>% 
  st_as_sf() %>% 
  st_transform("+proj=longlat +datum=WGS84") %>% 
  filter(name == "Nokomis-Hiawatha") %>% 
  st_simplify()

minneapolis <- tigris::places(state = "MN", class = "sf") %>% 
...
home_block_group_traffic <- read.csv("data/workshop_data/104606_Regional_Parks_Summer_2018_HW_workshop_home_block_groups_all.csv") %>% janitor::clean_names() %>% 
  filter(zone_name == "Nokomis-Hiawatha",
...
ggsave(promo_map, filename = "img/promo_map_nokomis.png", type = "cairo",
       dpi = 300)
